...
1. 実際の pandoc / typst(と、ビルド対象が PlantUML 図を参照している場合は plantuml)のバージョンを表示し、期待バージョン(BUILDING.md 参照)と異なる場合は警告を表示する(ビルドは継続する)。あわせて `SRC` の存在確認(章別ファイル分割の場合は `00-meta.md` と章ファイルの有無、参照図に対応する `.puml` の有無)を行う。
