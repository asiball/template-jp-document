...
3. フォアグラウンドで `$(SRC)`(単一ファイルならそのファイル、章別ファイル分割なら起動時点の全章ファイル。改訂履歴を別ファイル化している場合はそれも。参照中の図に対応する `.puml` と `template/plantuml.config` も)を 1 秒間隔でポーリングし、変更を検知するたびに lint →(`.revisions.md` / `revisions.md` の場合は YAML 変換)→ PlantUML 図の再変換(変更分のみ)→ pandoc を再実行して `build/obj/<name>.typ` を再生成する(再生成された `.typ` は上記の `typst watch` が拾って PDF に反映する)。章別ファイル分割の場合、章ファイルを 1 つだけ編集して保存しても、この仕組みにより全章ファイルが再度 pandoc に渡され `.typ` 全体が再生成されます。Markdown に図の参照を新しく追加した場合、保存すれば図の変換・PDF への反映は行われますが、その参照に対応する `.puml` ファイル自体のその後の編集を検知するには `make watch` の再起動が必要です(ポーリング対象が起動時点で固定されるため)。
