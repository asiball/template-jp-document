...
├── .github/workflows/build.yml       CI(PR ごとに lint・lint.sh の回帰テスト・make pdf でのサンプルビルドを検証)
...
**注意**: `SRC` のパスにスペースは使えません(Make の引数分割の制約のため)。スペースを含むパスを指定すると `make pdf` / `make watch` は明確なエラーメッセージで停止します(章別ファイル分割のディレクトリパスも対象です)。また、単一ファイルの `SRC` は `.md` 拡張子が必須です(改訂履歴の自動検出が `<name>.md` → `<name>.revisions.md` という命名規約に依存するため。`.md` 以外を指定すると明確なエラーで停止します)。
