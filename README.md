...
│   ├── list-diagram-refs.sh          Markdown が参照する .puml の列挙(Makefile が変換対象の決定に使用)
│   └── diagram-filter.lua            pandoc Lua フィルタ: .puml 参照を変換済み SVG のパスへ書き換え
...
2. `scripts/lint.sh` でビルド対象の Markdown を簡易チェック(`make lint` 単体は docs/ と examples/ の `*.md` 全件 + 章別ファイル分割ディレクトリすべてが対象)。
   - **エラー(ビルド停止)**: 見出しの手動採番(`# 1. foo` / `## 2) foo` のような「番号+ドット/括弧+空白」形式、`# 第1章 foo` / `# 1章 foo` のような「(第)N章/節/項」形式)、YAML フロントマターの `title:` 欠落・空、章別ファイル分割時に 00-meta.md 以外の章ファイルへ YAML フロントマターが混入していること、PlantUML 参照の不備(`/assets/diagrams/<name>.puml` 形式以外の参照、参照先ファイルの不存在)。
...
3. ビルド対象の Markdown が参照している PlantUML ソース(`/assets/diagrams/*.puml`)を `scripts/puml2svg.sh` で `build/diagrams/<name>.svg` へ変換する(変更されたものだけを再変換。図を参照していない文書では何もしないため、plantuml のインストールも不要)。
4. `pandoc --from markdown --to typst --standalone --template template/template.typ --lua-filter scripts/diagram-filter.lua` で Markdown を Typst ソースに変換(`build/obj/<name>.typ` に出力)。Lua フィルタが `.puml` への画像参照を手順 3 の変換済み SVG のパスへ書き換える。章別ファイル分割の場合は 00-meta.md を含む章ファイル一覧(ファイル名の辞書順)を複数の入力として pandoc に渡す(pandoc は複数入力ファイルを連結して 1 文書として処理する)。改訂履歴を別ファイル化している場合は、`revisions.md`(または `<name>.revisions.md`)を YAML に変換したうえで(YAML 方式ならそのまま)`--metadata-file` も付与される(下記「改訂履歴の別ファイル化」参照)
...
3. フォアグラウンドで `$(SRC)`(単一ファイルならそのファイル、章別ファイル分割なら起動時点の全章ファイル。改訂履歴を別ファイル化している場合はそれも。参照中の `.puml` と `template/plantuml.config` も)を 1 秒間隔でポーリングし、変更を検知するたびに lint →(`.revisions.md` / `revisions.md` の場合は YAML 変換)→ PlantUML 図の再変換(変更分のみ)→ pandoc を再実行して `build/obj/<name>.typ` を再生成する(再生成された `.typ` は上記の `typst watch` が拾って PDF に反映する)。章別ファイル分割の場合、章ファイルを 1 つだけ編集して保存しても、この仕組みにより全章ファイルが再度 pandoc に渡され `.typ` 全体が再生成されます。Markdown に `.puml` への参照を新しく追加した場合、保存すれば図の変換・PDF への反映は行われますが、その新しい `.puml` ファイル自体の編集を検知するには `make watch` の再起動が必要です(ポーリング対象が起動時点で固定されるため)。
...
2. Markdown から画像と同じ記法で **`.puml` ソースをそのまま参照**する。

```markdown
![在庫引当作成の処理シーケンス](/assets/diagrams/reservation-sequence.puml){width=75%}
```

ビルド時には、`make pdf` が参照中の `.puml` を `build/diagrams/<name>.svg` へ自動変換し(`scripts/puml2svg.sh`。変更されたものだけを再変換)、pandoc の Lua フィルタ(`scripts/diagram-filter.lua`)が参照パスを変換済み SVG へ書き換えます。執筆者が `build/` 配下のパスを意識する必要はありません。キャプション・図番号・幅指定は画像ファイルの場合と同様に機能します。
...
- **参照は `/assets/diagrams/<name>.puml` 形式(ルート絶対パス)で書いてください**。それ以外の形式や、参照先ファイルが存在しない場合は、`scripts/lint.sh` がエラーでビルドを停止します。
...
実例: `examples/sample-spec/04-api-spec.md` が `assets/diagrams/reservation-sequence.puml` を参照しています。
