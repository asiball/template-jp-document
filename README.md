...
1. 実際の pandoc / typst のバージョンを表示し、期待バージョン([BUILDING.md](BUILDING.md) の表)と異なる場合は警告を表示する(ビルドは継続する)。あわせて `SRC` の存在確認(章別ファイル分割の場合は `00-meta.md` と章ファイルの有無)も行う。
...
- **納品時は元 Markdown 一式を PDF と併せて納める**。Markdown 一式(`docs/` 配下の原稿・参照している `assets/images/` 配下の図版)を PDF と一緒に渡しておくと、受領側でのテキストの二次利用や、版間の差分確認がしやすくなります。
...
- pandoc / typst の想定バージョンとインストール方法
- Docker ビルド(`make pdf-docker`。本テンプレートの「正」のビルド方法)
- Typst バイナリのチェックサム検証・ベースイメージの digest 固定
