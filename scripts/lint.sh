...
#
# 除外: <name>.revisions.md(改訂履歴のパイプ表ファイル)は仕様書本文では
# ないため lint の対象外とする。YAML フロントマターを持たないので、除外
# しないと title チェックに誤って引っかかってしまう(引数なしの docs/*.md
# glob にもマッチするため、ここで確実にスキップする)。
# =============================================================================
set -eu
