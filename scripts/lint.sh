...
#
# --- モード判定 ---------------------------------------------------------
# 各ファイルは「親ディレクトリ名」と「ファイル名」の両方で「単一ファイル
# モード」か「章別ファイル分割モード」かを判定する。章別ファイル分割モード
# (章ファイル)とみなすのは、親ディレクトリ名が `docs` 以外で、かつファイル
# 名が Makefile の章 glob と同じ `[0-9][0-9]-*.md` パターンに一致するもの
# (例: docs/foo/01-bar.md)。それ以外(docs/ 直下のファイル、および数字
# プレフィックスを持たないファイル)は単一ファイルモードとして扱う。親
# ディレクトリ名だけで判定すると、docs/ 以外の場所に置いた単一ファイルの
# 仕様書を章ファイルと誤判定し、「フロントマター禁止」という真逆の指摘を
# してしまうため、ファイル名パターンも併用する。
...
#     みで終わる、空白のみ、またはクォートだけの `""` / `''`)の場合も
#     エラーにする。表紙・ヘッダの表示に title が必須なため。
#
...
for f in "$@"; do
	...
	# 章ファイル判定は「docs/ 直下以外」かつ「[0-9][0-9]-*.md」の両方を満たす
	# ものに限る(冒頭「モード判定」コメント参照)。
	is_chapter_mode=0
